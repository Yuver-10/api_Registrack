import { registerUser, loginUser, handleForgotPassword, handleResetPassword } from '../services/auth.services.js';
...
export const forgotPassword = async (req, res) => {
  try {
    ...
    res.status(200).json({ mensaje: "Si tu correo está registrado, recibirás un enlace para restablecer tu contraseña." });
  } catch (error) {
...
export const resetPassword = async (req, res) => {
  try {
    ...
    res.status(200).json({ mensaje: "Tu contraseña ha sido restablecida exitosamente." });
  } catch (error) {
    res.status(400).json({ mensaje: error.message });
  }
};
